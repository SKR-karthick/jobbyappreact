...
class Jobs extends Component {
  ...
  getJobs = async () => {
    ...
    if (employeeTypeList.length > 0) {
      queryParams.append('employmentType', employeeTypeList.join(','));
    }
    
    // Add minimum salary if selected
    if (minimumSalary) {
      // Convert LPA values: 1000000 -> 10, 2000000 -> 20, etc.
      const minSalaryNumber = parseInt(minimumSalary, 10) / 100000;
      queryParams.append('minimumSalary', minSalaryNumber);
    }
    ...
    const apiUrl = `http://localhost:4000/jobs?${queryParams.toString()}`
